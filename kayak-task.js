...
class KayakTask {
  ...
  startCronOnEvenMinute(locationURLs) {
    ...
    const cronMinute = Math.ceil(currentMinute / 5) * 5; // Round up the current minute to the nearest multiple of 5
    const nextCronTime = moment()
      .minute(cronMinute)
      .add(cronMinute <= currentMinute ? 5 : 0, 'minutes'); // If current minute is already a multiple of 5, add 5 minutes
